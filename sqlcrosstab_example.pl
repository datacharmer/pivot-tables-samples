...
if ($driver eq 'SQLite') {
    ...
    $dbh = DBI->connect("dbi:mysql:crosstab; host=localhost"
	    . ";mysql_read_default_file=$ENV{HOME}/sandboxes/msb_5_6_21/my.sandbox.cnf"  # only Unix. Remove this line for Windows
        ,  undef,  # username
           undef,  # password
          {RaiseError=>1, PrintError=> 0 }) 
}
...
for my $xt (($xtab1, $xtab2, $xtab3)) {
    ...
    # 
    # create a SQL example
    # 
    open SQL, ">$fname.sql" 
        or die "can't create $fname.sql\n";
    # print Dumper $xt;exit;
    my $query =  $xt->{query};
    print SQL  $query;

    close SQL;
    print "$fname.sql created\n";
